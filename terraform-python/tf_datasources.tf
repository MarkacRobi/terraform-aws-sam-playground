...
data "aws_cloudformation_export" "api_lambda_arn_cfn_exports" {
  # loop through api lambda variable and fetch the exports
  for_each   = toset(var.api_lambda_functions)

  # dependency on AWS SAM stack because cfn exports will not be available until SAM stack is created
