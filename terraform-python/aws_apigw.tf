...
# API Gateway
# ----------------------------------------------------------------------
resource "aws_api_gateway_rest_api" "sam_app_rest_api" {
  depends_on = [aws_cloudformation_stack.hello_world_api_sam_stack]
  name       = var.app_name
  ...
  body = templatefile("./files/api-def-v1.yaml", {
    ...
    get_data_uri               = "${var.lambda_invoke_uri_prefix}/${data.aws_cloudformation_export.api_lambda_arn_cfn_exports["hello-world"].value}/invocations"
  })
}
...
# ----------------------------------------------------------------------
resource "aws_api_gateway_deployment" "sam_app_rest_api_deployment" {
  depends_on  = [aws_api_gateway_rest_api.sam_app_rest_api]
  rest_api_id = aws_api_gateway_rest_api.sam_app_rest_api.id
  stage_name  = "v1"
  variables = {
...
resource "aws_api_gateway_usage_plan" "usage_plan" {
  ...
  api_stages {
    api_id = aws_api_gateway_rest_api.sam_app_rest_api.id
    stage  = aws_api_gateway_deployment.sam_app_rest_api_deployment.stage_name
  }
}
